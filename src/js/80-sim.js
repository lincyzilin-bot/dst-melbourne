...
function simulate(dt){
  P.dayT+=dt/DAY_LEN;
  if(P.dayT>=1){P.dayT-=1;P.day++;toast(`🌅 Day ${P.day} — ${P.day===4?'💧 WETLAND UNLOCKED! Head south-west!':'over Melbourne'}`);}
  ...
  for(const m of mobs){const dx=P.x-m.x,dz=P.z-m.z,d=Math.hypot(dx,dz)||0.01;m.x+=dx/d*4.5*dt;m.z+=dz/d*4.5*dt;m.mesh.position.set(m.x,0,m.z);m.atkT-=dt;if(d<1.6&&m.atkT<=0){m.atkT=1.2;P.hp-=8;toast('🕷️ Spider bite! -8 HP');}}
